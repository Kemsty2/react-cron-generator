...
export default class Cron extends Component {
  ...
  getHeaders() {
    return this.state.headers.map((d, index) => {
      return (
        <li
          key={index}
          className={this.state.selectedTab === d ? "active" : ""}
        >
          <a onClick={this.tabChanged.bind(this, d)}>{this.translate(d)}</a>
        </li>
      );
    });
  }
  ...
  getVal() {
    let val = cronstrue.toString(
      ...
      { throwExceptionOnParseError: false, locale: this.state.locale }
    );
